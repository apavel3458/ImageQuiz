...
function HelpSystem(helptext_span) {
	...
	printHelp('Click to draw a box around the large box in the grid (200ms by 5mm (0.5 mV)');
    };
...
function AmassView(canvas, naturalWidth, naturalHeight) {
    ...
    */
	...
	mVPerPixel = 5 / this.canvasHeightToImageHeight(Math.abs(grid_start_y - grid_end_y));
	...
	if (this.mode_ === 'grid_size' && grid_start_x !== undefined && grid_start_y !== undefined && grid_end_x !== undefined && grid_end_y !== undefined) {
		...
		writeLabel(ctx, '5 mm', Math.max(grid_start_x, grid_end_x) + textOffset, (grid_start_y + grid_end_y) / 2, 'left');
	    }
	}
	...
	writeLabel(ctx, '5mm', scale_x + offset + textOffset, scale_y / 2 + offset, 'left');
    }
	...
	if (msPerPixel === null || mVPerPixel === null) {
	    ...
	    return ((line.p1.y - line.p2.y) * mVPerPixel).toPrecision(4) + ' mm / ' + ((line.p2.x - line.p1.x) * msPerPixel).toPrecision(4) + ' ms';
	}
    }
	...
	writeLabel(ctx, vert_len.toPrecision(4) + ' mm',
...
function setOverlayPosition() {
	...
	case 'default':
	    ...
	    if (calibrateFail == true) alert ("Cannot detect ECG grid to calibrate measurements, use the 'Calibrate' button and draw a box around the large grid box (200ms by 5mm [0.5mV])");
	    break;
	}
	aug_view.drawAll();
    });
