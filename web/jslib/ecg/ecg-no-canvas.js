...
function HelpSystem(helptext_span) {
	...
	case HelpLevelEnum.DONE_FINISH_LINE:
	    printHelp("TIP: If the distance is off.");
	    break;
	...
	printHelp('Click to draw a box around the large box in the grid');
    };
...
// Constructor function.
function AugenmassView(canvas, naturalWidth, naturalHeight) {
    "use strict";
    ...
    */
	...
	if (pixel_len >= 2) {
	    var print_text = ((line.p1.y - line.p2.y) * mVPerPixel).toPrecision(4) + ' mV / ' + ((line.p2.x - line.p1.x) * msPerPixel).toPrecision(4) + ' ms';
	...
	writeLabel(ctx, print_text,
		   imageWidthToCanvasWidth(line.p1.x + line.p2.x)/2, imageHeightToCanvasHeight(line.p1.y + line.p2.y)/2 - 20,
...
function augenmass_init(naturalWidth, naturalHeight) {
    var $helptext = $overlayDiv.find('.helptext');
    help_system = new HelpSystem($helptext[0]);
    var $measure = $overlayDiv.find('.measure');
    aug_view = new AugenmassView($measure[0], naturalWidth, naturalHeight);
...
function setOverlayPosition() {
    ...
    augenmass_init(img.naturalWidth, img.naturalHeight);
