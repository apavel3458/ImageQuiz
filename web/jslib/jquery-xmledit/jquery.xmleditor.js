...
$.widget( "xml.xmlEditor", {
	...
	_init: function() {
		if (this.options.submitResponseHandler == null)
			this.options.submitResponseHandler = this.mySubmitResponseHandler;
	...
	submitXML: function() {
		...
		var self = this;
                var sendData = {'casexml': xmlString, 'exercisename': $("#exercisename").val()};
		$.ajax({
			...
			success : function(response) {
				...
				if (!outcome) {
					...
					$("." + submissionStatusClass).html("Failed to submit<br/>See errors at top").css("background-color", "#ffbbbb").animate({backgroundColor: "#ffffff"}, 1000);
					self.addProblem("Failed to submit xml document", "Server was not able to save the exercise");
				} else if (!isNaN(parseFloat(outcome)) && isFinite(outcome)) {
					self.xmlState.changesCommittedEvent();
					self.clearProblemPanel();
					$("#exerciseid").value(outcome);
				} else {
					self.xmlState.syncedChangeEvent();
					$("." + submissionStatusClass).html("Failed to submit<br/>See errors at top").css("background-color", "#ffbbbb").animate({backgroundColor: "#ffffff"}, 1000);
					self.addProblem("Failed to submit xml document.  The following is server's response:", outcome);
				}
			},
        ...
        mySubmitResponseHandler: function(response) {
                if (response.indexOf("Error:") === 0) {
                    return response;
                } else if (response.indexOf("id:") === 0) {
...
DocumentState.prototype.setXMLFromString = function(xmlString) {
	...
	if (this.domParser) {
		...
		if (parseError.length > 0){
			alert("Unable to load the editor because could not parse XML");
			$("#casexml").append("<textarea name=\"casexml\" id=\"casexml\" style=\"width: 100%; height: 400px;\">");
